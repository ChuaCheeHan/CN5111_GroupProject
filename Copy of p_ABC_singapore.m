...
for (a = 1:10)
     while p_BFS_enhanced_Singapore(bees(a,1), bees(a,2)) < 0
...
for (a = 1:10)
    [result dist_to_airport] = p_BFS_singapore(bees(a,1),bees(a,2));
    bees_fit(a) = p_BFS_enhanced_Singapore(bees(a,1),bees(a,2)) + dist_to_airport;
end
...
%%Employed bees stage
    for (a = 1:10)
        ...
        while (p_BFS_enhanced_Singapore(bees(a,1) + y_mod, bees(a,2) + x_mod) < 0)
            if (randomval < 4)
        ...
        [result dist_to_airport] = p_BFS_singapore(bees(a,1) + y_mod, bees(a,2) + x_mod);
        newFitness = p_BFS_enhanced_Singapore(bees(a,1) + y_mod, bees(a,2) + x_mod) + dist_to_airport;
    ...
    while (p_BFS_enhanced_Singapore(bees(p_idx,1) + y_mod, bees(p_idx,2) + x_mod) < 0)
        if (randomval < 4)
            randomval = randomval + 1;
        else randomval = 1;
        end
    ...
    [result dist_to_airport] = p_BFS_singapore(bees(p_idx,1) + y_mod,bees(p_idx,2) + x_mod);
    newFitness = p_BFS_enhanced_Singapore(bees(p_idx,1) + y_mod, bees(p_idx,2) + x_mod) + dist_to_airport;
    ...
    while p_BFS_enhanced_Singapore(bees(idx,1), bees(idx,2)) < 0
        bees(idx,1) = round(x_lo(1) + rand*(x_up(1) - x_lo(1)));
        bees(idx,2) = round(x_lo(2) + rand*(x_up(2) - x_lo(2)));
    end
    
    %update the fitness
    [result dist_to_airport] = p_BFS_singapore(bees(idx,1),bees(idx,2));
    bees_fit(idx) = p_BFS_enhanced_Singapore(bees(idx,1),bees(idx,2)) + dist_to_airport;
...
xval = bees(idx, 2)
iter
[result airport] = p_BFS_singapore(yval,xval);
airport
toc